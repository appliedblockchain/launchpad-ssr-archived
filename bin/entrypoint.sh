...
function run() {
  NODE_ENV=production node /home/node/build/server.js
}

TIME=2
CTR_ADDR_PATH=/home/node/contracts/build/contractAddresses.json
...
if [ -z "$CONTRACT_ADDRESSES" ]; then
  ...
  if [ -z "$CONTRACT_ADDRESSES" ]; then
    ...
    if [ "$HOSTNAME" = $LEADER ]; then
      echo "PROVIDER: $ETHEREUM_JSONRPC_ENDPOINT"
      mkdir -p /contracts/build/contracts
      PROVIDER="$ETHEREUM_JSONRPC_ENDPOINT" node /home/node/contracts/bin/deploy.js
